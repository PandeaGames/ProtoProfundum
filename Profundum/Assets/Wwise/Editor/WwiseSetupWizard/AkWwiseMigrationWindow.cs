    ...
    List<string> GetInstalledPlatformNames()
    {
		...
		try
		{
	        ...
	        foreach (FileInfo plugin in foundPlugins)
	        {
	            ...
	            else if (plugin.FullName.Contains(".meta") || plugin.FullName.Contains(".def") || plugin.FullName.Contains(".pdb") || Path.GetFileName(plugin.DirectoryName).Equals("plugins"))
	            {
	                continue;
	            }
	            else
